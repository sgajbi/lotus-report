...
Linux-resolved and Linux-enforced — pip environment markers make one exact closure
platform-specific, so on other platforms that full gate prints explicit not-evaluable rather than
comparing against a closure wrong for the host by construction (the lotus-gateway Ubuntu-only-gate
posture). The audit target uses its Linux container even from those hosts, so a security scan remains
an actual closure scan; the hook's narrow runtime check is not a claim that a non-Linux workstation
matches the entire Linux closure. `pyproject.toml` floors remain the compatibility statement; the
constraints file is the reproducibility statement and never a vulnerability-exception mechanism.
