...
| `report_revision_id` (`rrv3_...`; early rows carry `rrv2_...` from the superseded scheme that hashed the coverage claim) | Capture (identity binding) | Deterministic over the semantic request + stated source revisions + factual content under the versioned `fb1` boundary: re-captures of identical facts share it, a restated source cut changes it, a replay-clone inherits it verbatim. NULL on failed captures and on pre-identity history - never backfilled. Rides the render package (`render_context.report_revision_id`) and the custody block verbatim into Archive's document record; a resumed render RESOLVES the persisted render job with Render and adopts its outcome before any resubmission, so a package rebuilt by newer code never turns the resume into a package-hash conflict |
