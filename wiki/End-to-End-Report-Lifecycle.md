...
| Archive other 5xx / unclassified fault | `archive_execution_failed` | Retryable for portfolio review only | Same resolution-first replay as above (report#211; migration 014 backfills previously stranded rows). The ambiguity resolution exists because a committed-but-response-lost ingest under a NEW request id would defeat archive idempotency and duplicate the document. Families and rerender attempts without a resolution path stay non-retryable |
