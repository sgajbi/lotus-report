...
async def test_artifactless_render_failure_recovers_end_to_end_through_replay(tmp_path):
    ...
    # The reader-facing record translates the legacy 1.0.0 spelling to the
    # capability vocabulary, while the clone ROW inherited the source's
    # stored bytes verbatim - never a version/value pair no policy stamped.
    ...
    assert store.get_stored_lifecycle(cloned.snapshot_id) == {
        "policy_ref": "report-input-snapshot-standard",
        "policy_version": "1.0.0",
        "reproduction_availability": "rerender_from_snapshot",
        "lifecycle_authority": "test",
