...
| `lotus_report_job_runtime_last_items` | gauge | `item_state` | Latest bounded report-job worker pass counts for claimed, completed, retry-pending, and failed work items |
| `lotus_report_job_work_lease_events_total` | counter | `outcome` | Bounded recovered, exhausted, and stale-conflict report-work lease outcomes |
...
4. report-work lease recovery pressure from `lotus_report_job_work_lease_events_total`,
5. batch worker activity from `lotus_report_batch_runtime_last_items`,
6. scheduler materialization activity from `lotus_report_batch_scheduler_last_schedules`.
...
| Report-job worker inactivity | `lotus_report_job_runtime_last_items{item_state="completed"}` unchanged while runnable work items exist | warning | reporting-operations | Inspect report-job worker logs, expired leases, retry posture, and downstream source health |
| Report-work lease exhaustion | Increase in `lotus_report_job_work_lease_events_total{outcome="exhausted"}` | warning | reporting-operations | Inspect the failed source job, lifecycle event, downstream source health, and replay eligibility before operator replay |
| Stale report-work ownership | Increase in `lotus_report_job_work_lease_events_total{outcome="stale_conflict"}` | warning | reporting-operations | Inspect worker duration against lease settings and confirm no worker is completing work after losing its lease token |
