...
| Template / render-package identity | RETIRED from scheduling (report#283): template selection is resolved once at job ACCEPTANCE and persisted in the accepted document contract. Scheduling is PORTFOLIO-REVIEW-ONLY today - `BatchScheduleDefinition` carries no report type, the validator binds `report_family_id="portfolio_review"`, and dispatch always creates portfolio-review jobs; a schedule states only output formats, reporting currency, and composition options | `lotus-report` acceptance | `report_job.accepted_document_contract_json` via `accepted_document_contract` | Removed from scheduled-cycle identity and metadata |
