...
| Template / render-package identity | RETIRED from scheduling (report#283): template selection is resolved once at job ACCEPTANCE and persisted in the accepted document contract - schedules state only the requested policy (report type, output formats) | `lotus-report` acceptance | `report_job.accepted_document_contract_json` via `accepted_document_contract` | Removed from scheduled-cycle identity and metadata |
